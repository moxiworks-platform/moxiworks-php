...
class SoldSoldListingTest extends PHPUnit_Framework_TestCase {
    ...
    public function testFindThrowsExceptionWhenNoAuthorizationDataHasBeenSet() {
        ...
        $listing = \MoxiworksPlatform\SoldListing::find(['moxi_works_listing_id' => 'abc123', 'moxi_works_company_id' => 'abc123']);
        \VCR\VCR::eject();
    }
    ...
    public  function testeFindthrowsExceptionWhenNoCompanyParametersPassed() {
        ...
        \VCR\VCR::insertCassette('listing/find/success.yml');
        $listing = \MoxiworksPlatform\SoldListing::find(['moxi_works_listing_id' => 'abc123']);
        \VCR\VCR::eject();
    ...
    public function testReturnsSoldListingWhenFound() {
        $c = new \MoxiworksPlatform\Credentials('abc123', 'secret');
        \VCR\VCR::insertCassette('listing/find/success.yml');
        $listing = \MoxiworksPlatform\SoldListing::find(['moxi_works_listing_id' => 'abc123', 'moxi_works_company_id' => 'abc123']);
        $this->assertTrue(is_a($listing, '\MoxiworksPlatform\SoldListing'));
        \VCR\VCR::eject();
    }
    ...
    public function testSearchReturnsArrayWhenRequestSucceeds() {
        ...
        $results = \MoxiworksPlatform\SoldListing::search(['moxi_works_company_id' => '1234abcd', 'updated_since' => 1463595006]);
    ...
    public  function testSearchThrowsExceptionWhenNoMoxiWorksCompanyIdParametersPassed() {
        $this->setExpectedException('\MoxiworksPlatform\Exception\ArgumentException');
        \VCR\VCR::insertCassette('listing/find/success.yml');
        $results = \MoxiworksPlatform\SoldListing::search([ 'updated_since' => 1463595006]);
        \VCR\VCR::eject();
    }
    ...
    public function testSearchReturnsArrayWhenNoUpdatedSinceParameterPassed() {
        $c = new \MoxiworksPlatform\Credentials('abc123', 'secret');
        \VCR\VCR::insertCassette('listing/search/success.yml');
        $results = \MoxiworksPlatform\SoldListing::search(['moxi_works_company_id' => '1234abcd']);
        $this->assertTrue(is_array($results));
        \VCR\VCR::eject();
    }
